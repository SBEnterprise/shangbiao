...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		...
		<span class="r">共有数据：<strong>{{ $sum }}</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		@foreach($member_info as $v)
			<tr class="text-c">
				...
				<td>{{$v->id}}</td>
				<td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.html','10001','360','400')">{{$v->user_name}}</u></td>
				<td>
					@if ($v->sex == 1)
						男
					@elseif ($v->sex == 2)
				...
				<td class="td-manage">
					<a title="编辑" href="javascript:;" onclick="member_edit('编辑','{{url("admin/member/edit", $v->id)}}','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
	...
	<div style="float:right;">{{$member_info->links()}}</div>
	</div>
</div>
<!--_footer 作为公共模版分离出去-->
